...
/**
 * Derive a file name for a `[metadata.files]` entry of a python lock file.
 *
 * poetry.lock entries carry a `file` key, while pdm.lock entries carry a
 * `url` instead. We fall back to the basename of the URL so the resulting
 * `type: "file"` component always has a name (nameless components are invalid
 * per the CycloneDX schema).
 *
 * @param {object} fileEntry A single `[metadata.files]` entry.
 * @returns {string | undefined} The derived file name, or undefined when none can be derived.
 */
function derivePythonLockMetadataFileName(fileEntry) {
  if (typeof fileEntry?.file === "string" && fileEntry.file.trim()) {
    return fileEntry.file.trim();
  }
  if (typeof fileEntry?.url === "string" && fileEntry.url.trim()) {
    try {
      const name = basename(new URL(fileEntry.url.trim()).pathname);
      if (name) {
        return name;
      }
    } catch (_err) {
